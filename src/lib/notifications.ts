        ...
        if (pref.email && user.email && opts.emailSubject !== '') {
            ...
            if (locale !== 'en') {
                ...
                if (opts.type === 'announcement') {
                    ...
                    }, opts.imageUrl, opts.bodyHtml)
                } else if (opts.type === 'new_role') {
                    // Use the localized template — subject already resolved per-user above
                    html = opts.emailHtml  // already regenerated by notifyNewRole per locale via broadcastNotification meta
